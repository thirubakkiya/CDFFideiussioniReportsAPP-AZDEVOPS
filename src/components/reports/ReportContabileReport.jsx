...
export default function ReportContabileReport({ idBanca, reportType = 'contabile' }) {
  ...
  const filterTypes = reportType === 'operazioni' ? OPERATION_TYPES : CONTABILE_TYPES;
  const apiMethod = reportType === 'operazioni' 
    ? 'getOperazioniContabileReport' 
    : 'getReportContabile';
  ...
  const handleSearch = useCallback(async () => {
    ...
    try {
      ...
      const result = await ReportsService[apiMethod](request);
      setData(result || []);
  ...
  }, [contoAddebito, executionDate, selectedType, idBanca, apiMethod]);
  ...
  const handleExport = useCallback(async () => {
    try {
      ...
      const fileName = reportType === 'operazioni' 
        ? 'OperazioniContabileReport' 
        : 'ReportContabileReport';

      await ExportService.exportToXLS(data, fileName, COLUMNS);
    } catch (err) {
      try {
        const fileName = reportType === 'operazioni' 
          ? 'OperazioniContabileReport' 
